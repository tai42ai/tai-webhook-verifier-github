"""tai-webhook-verifier-github — the GitHub webhook-signature verifier plugin.

A per-provider :class:`~tai_contract.webhooks.WebhookVerifier` for the TAI
...
Its only dependency is ``tai-contract`` (the interface it registers through); the
...
tai_app.webhook_verifiers.register("github", GitHubWebhookVerifier())
